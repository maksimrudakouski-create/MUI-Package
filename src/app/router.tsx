...
//
// MUI note: ThemeProvider (src/theme/ThemeProvider.tsx) mounts CssBaseline and
// the light/dark color schemes. DevBar reads the active scheme with
// useColorScheme(), so it needs no props from here.
...
/**
 * Designers type their screens as React's `ComponentType`, which is the type
 * they already know. TanStack's `RouteComponent` is narrower — it excludes
 * class components. Every screen in /flows is a function component, so the
 * cast is safe, and doing it here keeps a TanStack type out of the
 * designer-owned routes.tsx. Same reason as the ":id" -> "$id" translation
 * above: the plumbing lives on the dev side of the seam.
 */
const toRouteComponent = (c: FlowRoute["component"]) => c as RouteComponent;
